...
(function (root) {
  ...
  function emptyMessage(label) {
    return '<p class="catalog-empty" style="grid-column:1/-1;text-align:center;color:#94a3b8;font-size:.95rem;padding:24px">' + escapeHtml(label) + "</p>";
  }
  ...
  function renderTours(mount, destCfg) {
    ...
    mount.innerHTML = tours
      .map((tour) => {
        ...
        const dates = Array.isArray(tour.dates) && tour.dates.length ? tour.dates.slice(0, 3).join(" · ") : "";
        const badge = tour.badge && tour.badge.text ? '<span class="tbadge ' + escapeHtml(tour.badge.type || "hot") + '">' + escapeHtml(tour.badge.text) + "</span>" : "";
        return (
          "" +
          '<div class="tcard reveal" data-cat="' +
          escapeHtml(region) +
          '" data-href="' +
          escapeHtml(detail) +
          ...
          '<div class="tc-img">' +
          '<img src="' +
          escapeHtml(imgSrc(base, tour.img)) +
          '" alt="' +
          escapeHtml(tour.title || "") +
          '" data-fallback="' +
          escapeHtml(tour.imgFb || "linear-gradient(135deg,#219EBC,#1B2A4A)") +
          '">' +
          '<div class="tc-badges">' +
          badge +
          (tour.duration ? '<span class="tbadge dur">' + escapeHtml(tour.duration) + "</span>" : "") +
          "</div>" +
          ...
          "</div>" +
          '<div class="tc-footer">' +
          '<div class="tc-price"><span class="new-pr">' +
          priceTx +
          '</span><span class="per-pr">/người</span></div>' +
          '<button class="btn-book" type="button" data-book-tour="' +
          escapeHtml(tour.title || "") +
          '">Đặt Tour</button>' +
          "</div>" +
          "</div>" +
  ...
  function renderCombos(mount) {
    ...
    mount.innerHTML = combos
      .map((combo) => {
        ...
        const includes = incs.map((c) => '<li><i class="fas ' + escapeHtml(c.icon || "fa-check") + '"></i> ' + escapeHtml(c.title || "") + "</li>").join("");
        return (
          "" +
          '<div class="ccard reveal" data-href="' +
          escapeHtml(detail) +
          '" style="cursor:pointer">' +
          '<div class="cc-img">' +
          '<img src="' +
          escapeHtml(imgSrc(base, combo.img)) +
          '" alt="' +
          escapeHtml(combo.name || "") +
          '" data-fallback="linear-gradient(135deg,#0096C7,#0077B6)">' +
          (combo.badge ? '<div class="cc-tag">' + escapeHtml(combo.badge) + "</div>" : "") +
          (save ? '<div class="cc-save">' + escapeHtml(save) + "</div>" : "") +
          "</div>" +
          '<div class="cc-body">' +
          '<div class="cc-dest"><i class="fas fa-map-marker-alt"></i> ' +
          escapeHtml(combo.dest || "") +
          (combo.duration ? " · " + escapeHtml(combo.duration) : "") +
          "</div>" +
          '<div class="cc-name">' +
          escapeHtml(combo.name || "") +
          "</div>" +
          ...
          "</div>" +
          '<button class="btn-combo" type="button" data-book-tour="' +
          escapeHtml(combo.name || "") +
          '">Đặt Combo</button>' +
          "</div>" +
          "</div>" +
  ...
  function renderServices(mount) {
    ...
    mount.innerHTML = services
      .map((service, idx) => {
        ...
        return (
          "" +
          '<div class="srv reveal" data-href="' +
          escapeHtml(detail) +
          '" style="cursor:pointer">' +
          '<div class="srv-top ' +
          gClass +
          '">' +
          '<img src="' +
          escapeHtml(imgSrc(base, service.img)) +
          '" alt="' +
          escapeHtml(service.name || "") +
          '" data-fallback="' +
          escapeHtml(service.imgFallback || "linear-gradient(135deg,#1B2A4A,#E85D04)") +
          '">' +
          '<i class="' +
          escapeHtml(icon) +
  ...
  function bootstrap() {
    ...
    Promise.all(work)
      .then(() => {
        ...
        if (tourMount) {
          renderTours(tourMount, destCfg);
          attachImageFallbacks(tourMount);
        }
        if (comboMount) {
          renderCombos(comboMount);
          attachImageFallbacks(comboMount);
        }
        if (serviceMount) {
          renderServices(serviceMount);
          attachImageFallbacks(serviceMount);
        }
