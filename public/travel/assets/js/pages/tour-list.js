...
(function () {
  ...
  function render(TOURS) {
    ...
    document.getElementById("listCount").innerHTML = "Hiển thị <strong>" + list.length + "</strong> tour" + (activeFilter !== "all" ? " (đã lọc)" : "");
  ...
  function getDestLabel(t) {
    const cfg = (window.SaigonData && window.SaigonData.destCfg) || {};
    const key = t.dest && t.dest[0];
    return (cfg[key] && cfg[key].label) || t.destName || "";
  }
    ...
    return `
      <div class="tc" data-id="${t.id}">
        ...
        <div class="tc-body">
          ...
          <div class="tc-meta">
            <div class="tc-meta-row"><i class="fas fa-map-marker-alt"></i>${getDestLabel(t)}${t.dep ? " · Khởi hành từ " + t.dep : ""}</div>
