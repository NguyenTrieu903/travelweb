...
(function () {
  ...
  function init() {
    ...
    currentTour = tours.find((tour) => tour.id === id) || tours.find((tour) => tour.id === legacyIdMap[id]) || tours[0];
  ...
  function renderPage(tour, tours) {
    ...
    if (heroSummaryEl) {
      heroSummaryEl.textContent = tour.summary || "";
      heroSummaryEl.hidden = !tour.summary;
    }
    ...
    setHTML(
      ...
      [
        ...
        { icon: "fa-map-marker-alt", label: "Điểm đến", val: getDestLabel(tour) },
  ...
  function buildBadges(tour, className) {
    return tour.badge ? `<span class="${className} ${tour.badge.type}">${tour.badge.text}</span>` : "";
  }
      ...
      <div class="itin-day${index === 0 ? " open" : ""}">
        ...
        <div class="itin-body">
          <div class="itin-inner">
            <ul class="itin-acts">${arrayOf(day.activities || day.acts)
              .map((item) => `<li><i class="fas fa-circle"></i>${item}</li>`)
              .join("")}</ul>
            <div class="itin-meals"><span class="meal-lbl">Bữa ăn:</span>${arrayOf(day.meals)
              .map((meal) => `<span class="meal-tag">${meal}</span>`)
              .join("")}</div>
          </div>
        </div>
      </div>`,
  ...
  function getDestLabel(tour) {
    ...
    return (cfg[key] && cfg[key].label) || tour.destName || "Tour du lịch";
  }
